...
function buildContactItem({ id, value }) {
  const item = document.querySelector(`#${ id }`);
  const itemLink = document.createElement('a');
  itemLink.id = `contact-${ id }`;
  itemLink.className = 'contact-item';
  itemLink.textContent = value;
  itemLink.href = `mailto:${ value }`;
  item.appendChild(itemLink);
}

function buildContactName(personInformation) {
...
async function buildCard() {
  let url = window.location.search; // TODO: new URL()
  console.log(window);
  url = url.split('=');
  id = url[1];
  console.log(url);
  ...
  try {
    ...
    // TODO: buildContactName()
    const nameCard = document.querySelector('#name-contact');
    nameCard.textContent = personInformation.nome + ' ' + personInformation.cognome;

    buildContactItem({ id: 'email', value: personInformation.email });
    /*const email = document.querySelector('#email');
    const emailLink = document.createElement('a');
    emailLink.id = 'emailLink'; // TODO: email-link
    ...
    buildContactItem({ id: 'tel', value: personInformation.tel });
    ...
    // TODO: buildContactFavoriteVideo()
...
function showMod() {
  ...
  imgprofile.src = document.querySelector('#img-profile').src;
  ...
  changePanelInfo.appendChild(createInput('text', document.querySelector('#img-profile').src, 'imgProfileNew'));
...
function saveMod() {
  ...
  newPerson.fotoprofilo = document.querySelector('#img-profile').src;
